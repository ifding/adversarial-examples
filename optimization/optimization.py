...
def calc_gradients(
    ...
    #print(-spec.mean+spec.rescale[0])
    #print(-spec.mean+spec.rescale[1])
    ...
    loss2 = tf.sqrt(tf.reduce_mean(tf.square(true_image - input_image)))  #L_2
    #loss2 = tf.reduce_sum(tf.maximum(0.0,tf.abs(true_image - input_image)))  #L_infty

    probs, variable_set = get_model(sesh, true_image, model_name)
    ...
    save_times = int((max_iter - 1) / save_freq + 1)
    ...
    init_varibale_list = set(tf.global_variables()) - variable_set
    ...
    for (indices, labels, names, images) in image_producer.batches(sesh):
        ...
        for cur_iter in range(max_iter):
            ...
            if (cur_iter + 1) % save_freq == 0:
                ...
                for i in range(len(indices)):
                    gradient_record[int((cur_iter + 1)/save_freq) - 1][indices[i]] = noise_diff[i]
...
def save_file(sesh, image_producer, noise, data_spec, args):
    save_times = int((args.num_iter - 1)/args.save_freq) + 1
